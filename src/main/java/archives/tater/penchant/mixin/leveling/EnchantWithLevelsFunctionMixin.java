...
public class EnchantWithLevelsFunctionMixin {
    ...
    private static ItemStack addProgress(ItemStack original, @Local(argsOnly = true) LootContext context) {
        ...
        if (!(context.getResolver() instanceof HolderLookup.Provider registries)) return original;

        EnchantmentProgress.addRandomProgress(original, registries, context.getRandom());
